...
>- **Target Runtime & Performance Roadmap - Unity 7.0 & C# 14 Evolution:** We are targeting a stable Unity 7.0 runtime in the long run to natively leverage advanced modern C# capabilities with zero developer overhead. A example of this is utilizing C# 14’s params ReadOnlySpan<T> support. This swaps standard heap-allocated params T[] arrays for stack-allocated spans behind the scenes. Developers retain the familiar, clean params invocation syntax while completely eliminating hidden garbage collection (GC) allocations on hot paths.
